...
app.get('/blog', function(request, response) {
    Promise.all([Article.findAll({ include: [Tag] }), Tag.findAll(), Category.findAll()])
...
app.get('/blog/tag/:id', function(request, response) {
    Promise.all([Tag.findById(request.params.id, { include: [Article] }), Tag.findAll(), Category.findAll()])
        .then(x => response.render('blog.html', {
            title: x[0].name,
            sub_title: '标签',
...
app.post('/api/blog/publish', function(request, response) {
    var article;
    Promise.all([
            Promise.all(request.body.tags.map(x => Tag.findOne({ where: { name: x } }))),
            Promise.all([Article.create({ title: request.body.title, summary: request.body.summary, content: request.body.content })
                .then(x => article = x),
                Category.findOne({ where: { name: request.body.cate } })
            ]).then(x => article.setCategory(x[1]), x => response.json({ result: "failed", message: x[0] + x[1] })),
        ])
        .then(x => article.setTags(x[0]), x => response.json({ result: "failed", message: x[0] + x[1] }))
        .then(() => response.json({ result: "success", url: "/blog/" + article.id }), x => response.json(x));
});
// Get tags
...
app.post('/api/blog/categories', function(request, response) {
    Category.create({ name: request.body.name, title: request.body.title })
        .then(x => response.json({ result: "success", url: "" }), x => response.json(x));
});
