...
(function(global, factory) {
    ...
    function findFirst(pred, from, to) {
        for (;;) {
            ...
            if (pred(mid)) { to = mid } else { from = mid }
        }
    }
        ...
        if (order) {
            ...
            var ch = findFirst(function(ch) {
                ...
                if (box.top > y) {
                    ...
                    return true
                } else if (box.bottom <= y) { return false } else if (box.left > x) { return true } else if (box.right < x) { return false } else { return (x - box.left < box.right - x) }
            }, begin, end)
    ...
    function updateWidgetHeight(line) {
        if (line.widgets) {
            for (var i = 0; i < line.widgets.length; ++i) { line.widgets[i].height = line.widgets[i].node.parentNode.offsetHeight }
        }
    }
        ...
        function walk(node) {
            if (node.nodeType == 1) {
                ...
                if (markerID) {
                    ...
                    if (found.length && (range = found[0].find())) { addText(getBetween(cm.doc, range.from, range.to).join(lineSep)) }
                    return
                }
